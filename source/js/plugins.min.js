(function(d){d.fn.asyncFileUpload=function(g){var a=d.extend({},{afterSubmit:function(){},success:function(a){},error:function(a){}},g);return this.each(function(){var f=d(this),e=new FormData(this),b=f.find(":file")[0].files,c=0;if(30<=b.length)a.error("Cannot upload more than 30 images at a time.");else{for(;c<b.length;c++){if(5<b[c].size/1024/1024){a.error('The size of image "'+b[c].name+'" exceeds 5 MB.');return}if("image/jpeg"!==b[c].type&&"image/png"!==b[c].type){a.error('Image "'+b[c].name+'" is not a valid image (jpeg/png).');return}}e.append("host",window.location.hostname);e.append("protocol",window.location.protocol);d.ajax({url:f.attr("action"),type:"POST",success:a.success,error:a.error,data:e,dataType:"json",crossDomain:!0,cache:!1,contentType:!1,processData:!1});a.afterSubmit();return this}})}})(jQuery);
